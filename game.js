...
function renderNave() {
  ctx.save();
  ctx.translate(
    naveConfig.x + naveConfig.width / 2,
    naveConfig.y + naveConfig.height / 2
  );
...
function renderTempo() {
  if (gameConfig.jogando) {
    gameConfig.tempoAtual = (
      (new Date().getTime() - gameConfig.tempoInicial) /
      1000
    ).toFixed(1);
...
function renderPontuacao() {
  ...
  ctx.fillStyle = "#FFF";
  ctx.fillText(
    `Pontos: ${gameConfig.pontuacao + gameConfig.pontuacaoTempo}`,
    canvas.width - 100,
    canvas.height - 8
  );
  ctx.restore();
...
function renderDificuldade() {
  ...
  ctx.fillStyle = "#FFF";
  ctx.fillText(
    `Dificuldade: ${gameConfig.dificuldade}`,
    canvas.width - 140,
    30
  );
  ctx.restore();
...
function renderAsteroides() {
  if (gameConfig.asteroides.length === 0) {
    for (let i = 0; i < gerarNumeroAleatorio(maximoAsteroides); i += 1) {
      ...
      let newY =
        gerarNumeroAleatorio(canvas.height - asteroideConfig.height) -
        canvas.height;
      ...
      const minimoVelocidade =
        asteroideConfig.velocidade +
        asteroideConfig.velocidade * dificuldadeVelocidade;
      let velocidade = Math.ceil(
        gerarNumeroAleatorio(minimoVelocidade * 1.2, minimoVelocidade)
      );
  ...
  if (gameConfig.jogando) {
    ...
    ctx.restore();
    gameConfig.asteroides = gameConfig.asteroides.filter(
      (asteroide) => asteroide.y <= canvas.height
    );
  }
}
...
function detectarColisao(asteroide) {
  if (
    naveConfig.x + naveConfig.width > asteroide.x &&
    naveConfig.x < asteroide.x + asteroide.width &&
    naveConfig.y + naveConfig.height > asteroide.y &&
    naveConfig.y < asteroide.y + asteroide.height
  ) {
